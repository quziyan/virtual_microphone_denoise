...
**安装包:** `VibeCodingVirMic-Installer-1.0.3.pkg`

- **修复:** 安装新版本时,若旧版本正在运行,自动关闭它(否则旧进程仍在内存里运行旧代码);安装后若刚才在运行,则自动启动新版。

## v1.0.2 — 2026-06-09

**安装包:** `VibeCodingVirMic-Installer-1.0.2.pkg`

- **上报增强:** 离线时事件本地排队(落盘,退出也不丢),不阻塞、不报错;有网时按发生时间顺序自动补报。
- **新增上报字段:** IP 地址、版本号(随事件发生时刻记录)。
